...
import '../../modules/travel/screens/travel_list_screen.dart';
import '../../modules/travel/screens/travel_player_screen.dart';
...
class AppRoutes {
  ...
  static AppRouteData resolve(String route, SessionModel session) {
    switch (route) {
      ...
      case travel:
        final roleCode = RoleMapper.normalize(session.role);
        return AppRouteData(
          title: 'Logistique & Voyages',
          builder: (_) => roleCode == RoleMapper.player
              ? TravelPlayerScreen(
                  clubId: session.clubId ?? 'odin-club-1',
                  playerId: session.userId,
                )
              : TravelListScreen(clubId: session.clubId ?? 'odin-club-1'),
          showAppBar: true,
          usePadding: false,
        );
      default:
