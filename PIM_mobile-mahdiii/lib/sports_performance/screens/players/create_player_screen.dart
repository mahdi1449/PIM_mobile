  ...
  Future<void> _submit() async {
    if (_formKey.currentState!.validate() && _selectedDateOfBirth != null) {
      try {
        ...
        Player? result;
        if (widget.playerToEdit != null) {
          result = await ref.read(playerFormProvider.notifier).updatePlayer(widget.playerToEdit!.id!, playerModel);
        } else {
          result = await ref.read(playerFormProvider.notifier).createPlayer(playerModel);
        }
        ...
        if (mounted) {
          if (result != null) {
            ...
            // Error is handled by StateNotifierProvider's async error state or by the exception caught below
            // But we show a generic error if result is null and no exception was thrown to the outer catch
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Failed to save player. Please check your connection.'),
